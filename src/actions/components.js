...
export const addChild = ({ title, childType, HTMLInfo }: { title: string, childType: string, HTMLInfo: object }) => (
  dispatch: any,
) => {
...
export const changeComponentFocusChild = ({ componentId, childId }: { componentId: number, childId: number }) => (
  dispatch: any,
) => {
  dispatch({
...
export const updateHtmlAttr = ({ attr, value }: { attr: string, value: string }) => (dispatch: any) => {
  dispatch({
...
export const updateChildrenSort = ({ newSortValues }: { newSortValues: any }) => (dispatch: any) => {
  dispatch({
...
=======
>>>>>>> f213db0... changed imports on many files for application deployment. Now builds
  });
};
