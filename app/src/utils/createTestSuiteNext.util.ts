...
async function createJestTsconfigJsonFile(path: string, appName: string) {
  const filePath = `${path}/${appName}/jest.tsconfig.json`;
  const data = `
{
  "compilerOptions": {
    "module": "commonjs",
...
async function createEnzymeFile(path: string, appName: string) {
  const filePath = `${path}/${appName}/enzyme.js`;
  const data = `const Adapter = require('enzyme-adapter-react-16');
...
async function createComponentTests(
  ...
  components.forEach((page) => {
    let importString = '';
    if (page.isPage) {
      importString = `import ${capitalize(page.name)} from "../pages/${page.name}";`;
      data += importString;
    } else {
      importString = `import ${capitalize(page.name)} from "../components/${page.name}";`;
      data += importString;
    }
  });
