    ...
    for (const element of array) {
      if (element.name !== 'separator') {
        ...
        if (elementType !== 'button') {
          componentsToRender.push(<Box component={elementType} style={elementStyle} key={key} id={childId}>{innerText}{renderedChildren}</Box>);
        } else componentsToRender.push(<Button key={key} id={childId} style={elementStyle} color={'primary'}>{innerText}{renderedChildren}</Button>);
        key += 1;
      }
    }
