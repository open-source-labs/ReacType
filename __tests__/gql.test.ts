...
describe.skip('GraphQL tests', () => {
  ...
  describe.skip('Testing GraphQL query', () => {
    it('getAllProjects should return more than 1 project by default', () =>
      request(server)
        ...
        .expect(200)
        .then((res) =>
          expect(res.body.data.getAllProjects.length).toBeGreaterThanOrEqual(1)
        ));
    it('getAllProjects should return projects that matches the provided userId', () =>
      request(server)
        .post('/graphql')
        .set('Content-Type', 'application/json')
        .send({
        ...
        .expect(200)
        .then((res) =>
          expect(res.body.data.getAllProjects[0].userId).toBe(
            '604d21b2b61a1c95f2dc9105'
          )
        ));
  });
  // GraphQL Mutation

  describe.skip('Testing GraphQL mutation', () => {
    // Add likes
    ...
    // Publish project
    it('Should set the "published" on the project document to TRUE', () =>
      request(server)
        .post('/graphql')
        .set('Content-Type', 'application/json')
        .send({
        ...
        .expect(200)
        .then((res) =>
          expect(res.body.data.publishProject.published).toBe(true)
        ));
    it('Should set the "published" on the project document to FALSE', () =>
      request(server)
        .post('/graphql')
        .set('Content-Type', 'application/json')
        .send({
        ...
        .expect(200)
        .then((res) =>
          expect(res.body.data.publishProject.published).toBe(false)
        ));
    // Make copy
    it('Should make a copy of an existing project and change the userId and userName', () =>
      request(server)
        .post('/graphql')
        .set('Content-Type', 'application/json')
        .send({
