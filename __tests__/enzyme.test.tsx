...
import Adapter from '@wojtekmaj/enzyme-adapter-react-17'; //installed this, old dependency used React 16
...
xdescribe('Test the BottomTabs component', () => {
...
xdescribe('Test AppContainer container', () => {
  const target = shallow(
    <Provider store={store}>
      <AppContainer />
    </Provider>
  ...
  xit('Should render CustomizationPanel', () => {
    expect(target.find(customizationPanel)).toHaveLength(1);
  });
});

// testing for NavBar component
xdescribe('Test NavBar component', () => {
  const props = {
    setTheme: jest.fn(),
    isThemeLight: jest.fn()
  };
  ...
  xit('Should render 2 buttons: "Clear Canvas", "Dark Mode"', () => {
    ...
    expect(target.find('.navbarButton').at(0).text()).toEqual('Clear Canvas');
    expect(target.find('.navbarButton').at(1).text()).toEqual('Dark Mode');
  });

  it('Should render "Login" button', () => {
    const wrapper = shallow(<LoginButton />);
    expect(wrapper).toHaveLength(1);
    expect(wrapper.find('.navbarButton')).toHaveLength(1);
  });
});

xdescribe('Test LeftContainer container', () => {
  const target = shallow(
    <Provider store={store}>
      <LeftContainer />
    </Provider>
