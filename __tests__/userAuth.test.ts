...
describe('User Authentication tests', () => {
  ...
  describe('/login', () => {
    ...
    describe('POST', () => {
      it('responds with status 200 and json object on verified user login', () => request
        .post('/login')
        .set('Accept', 'application/json')
        .send(user)
        .expect(200)
      ...
      it('responds with status 400 and json string on invalid user login', () => request
        .post('/login')
        .send({ username: 'wrongusername', password: 'wrongpassword' })
        .expect(400)
        .expect('Content-Type', /json/)
        .then((res) => expect(typeof res.body).toBe('string')));
      it("returns the message 'No Username Input' when no username is entered", () => request
        .post('/login')
        .send({
          username: '',
      ...
      it("returns the message 'No Password Input' when no password is entered", () => request
        .post('/login')
        .send({
      ...
      it("returns the message 'Invalid Username' when username does not exist", () => request
        .post('/login')
        .send({
          username: 'l!b',
          password: 'test',
    ...
    it("returns the message 'Incorrect Password' when password does not match", () => request
      .post('/login')
      .send({
        username: 'test',
        password: 'password1!',
  ...
  describe('/updatePassword', () => {
    describe('PATCH', () => {
      ...
      it('responds with status 200 and json string on valid password update (Success)', () => request
      ...
      it('responds with status 400 and json string if no password is provided (Password is required.)', () => request
        .patch('/updatePassword')
        .set('Accept', 'application/json')
      ...
      it('responds with status 404 and json string if user is not found (User not found.)', () => request
        .patch('/updatePassword')
        .set('Accept', 'application/json')
      ...
      it('responds with status 400 and json string the provided password is the same as the current password (New password must be different from the current password.)', () => request
        .patch('/updatePassword')
        .set('Accept', 'application/json')
        .send({
          username: testUsername,
        ...
        .expect('Content-Type', /json/)
        .expect(400)
        .then((res) => expect(res.body.error).toBe('New password must be different from the current password.')));
    });
  });
});
