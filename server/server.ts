  ...
  client.on('disconnecting', async () => {
    try {
      ...
      if (!Object.keys(roomLists[roomCode].userList).length) {
        const { meetingId } = roomLists[roomCode];
        const options = {
          method: 'POST',
          headers: {
            Authorization: process.env.VITE_VIDEOSDK_TOKEN,
...
app.get('/loggedIn', sessionController.isLoggedIn, (req, res) => res.status(200).json(res.locals.loggedIn));
...
app.patch(
  ...
  cookieController.setSSIDCookie,
  sessionController.startSession,
  (req, res) => res.status(200).json({ sessionId: res.locals.ssid, message: 'Success' }),
);
